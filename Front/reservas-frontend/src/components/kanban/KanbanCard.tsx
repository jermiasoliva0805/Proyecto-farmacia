...
interface KanbanCardProps {
  ...
  // mostrarAlertaDemora ahora refleja pedido.estaDemorado (subestado flag)
          ...
          <div className="space-y-1 sm:space-y-2">
            ...
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="flex items-center gap-1">
                ...
                {mostrarAlertaDemora && (
                  <div title="Pedido demorado (+48hs hábiles)">
                    <AlertTriangle
            ...
            {/* Badge subestado DEMORADO — visible junto al estado principal */}
            {mostrarAlertaDemora && (
              <div className="flex items-center gap-1 bg-orange-50 border border-orange-200 rounded-full px-2 py-0.5 w-fit">
            ...
            {/* Indicadores de armado (solo Operarios) */}
            {user?.rol === 'Operario' && !pedido.fechaInicioArmado && (
            ...
            {pedido.fechaEntregaEstimada &&
              (typeof pedido.fechaEntregaEstimada === 'string'
            ...
            {/* Intentos fallidos en Despachando */}
            {pedido.idEstadoDePedido === 5 && pedido.intentosEntregaFallida > 0 && (
              <div
                className={`rounded p-1 sm:p-2 text-[10px] sm:text-xs border ${
            ...
            {/* Intentos fallidos en Entrega fallida */}
            {pedido.idEstadoDePedido === 8 && pedido.intentosEntregaFallida > 0 && (
              ...
              >
                {pedido.intentosEntregaFallida >= 3 ? (
                  ...
                  <>
                    ⚠️ {pedido.intentosEntregaFallida}/3 intento(s) fallido(s) —{' '}
                    {3 - pedido.intentosEntregaFallida} reintentos disponibles
                  </>
                )}
              </div>
            )}
