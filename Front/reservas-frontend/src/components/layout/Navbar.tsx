...
export const Navbar: React.FC<NavbarProps> = ({ onMenuClick }) => {
  ...
  // Hook activo — obtiene pedidos demorados con polling cada 30s
  const { notifications, loading, hasUnread, markAsRead } =
    useDemoradoNotifications(30000);
      ...
      <div className="px-4 h-full">
        <div className="flex items-center justify-between h-full">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <button
              ...
              title={
                notifications.length > 0
                  ? `${notifications.length} pedido(s) demorado(s)`
                  : 'Sin pedidos demorados'
              }
            >
              <Bell className="w-5 sm:w-6 h-5 sm:h-6 text-white" />

              {/* Indicador de no leído (pulso) */}
              {hasUnread && (
                <span className="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-yellow-400 rounded-full animate-pulse" />
              )}
            ...
            <div className="h-8 w-px bg-blue-700 mx-1 hidden sm:block" />
